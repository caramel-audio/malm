...
<section class="h-full flex flex-col">
	...
	<button
		...
		class="m-3 border border-dashed flex flex-col items-center justify-center gap-2 py-6 text-xs cursor-pointer transition-colors {dragOver
			...
			: 'border-[#3a3a3a] text-[#6a6a6a] hover:border-[#c8a84b] hover:text-[#c8a84b]'}"
		ondrop={onDrop}
	...
	<ul class="flex-1 overflow-y-auto">
		{#each files.list as f, i (f.id)}
			<li
				class="flex items-center gap-3 px-3 py-1.5 border-b border-[#1f1f1f] hover:bg-[#1f1f1f] {dragSrcIndex === i ? 'opacity-40' : ''}"
				draggable="true"
			...
			>
				<span class="text-[#3a3a3a] cursor-grab select-none">⠿</span>
				<span class="flex-1 truncate text-xs">{f.name}</span>
				{#if f.artist}
					<span class="text-[#6a6a6a] shrink-0 text-xs truncate max-w-24">{f.artist}</span>
				{/if}
				<span class="text-[#6a6a6a] shrink-0 tabular-nums text-xs">{formatDuration(f.duration)}</span>
				<button
					class="text-[#6a6a6a] hover:text-[#c8a84b] text-xs shrink-0"
