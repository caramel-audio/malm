...
4. `loudness.ts` — push-based `LoudnessMeter` (`feed()` / `finish()`): K-weighted LUFS, custom O(N) sliding windows for momentary/short-term, custom EBU R128 gating for integrated. The gating pass returns the surviving mean square _per channel_, so `integrated` and the L/R `balance` series (short-term window, silence-gated, stereo only) come off the same blocks. Filter state and the partial 100 ms step carry across `feed()` calls, so chunking never changes the result (`loudness.spec.ts` pins this). `WaveformAccumulator` collects the peak file.
...
- `Results.svelte` — band/loudness-type selectors (momentary / short-term / L/R balance — the last only when stereo results exist), "normalize to quietest" toggle (hidden in balance view, where a per-file offset cancels out), renders one `Plot` per file; the pinned file's plot sits outside the scroll area. Controls are a left sidebar on desktop, two-column top bar on mobile
- `Plot.svelte` — D3-based loudness timeline with playhead and click-to-seek; in balance mode it swaps to a fixed ±6 dB symmetric axis (fixed, not autoscaled, so tracks stay comparable) with an L/R centre line; reports hover values to the transport bar and owns no playback controls of its own
